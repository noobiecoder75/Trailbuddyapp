...
export const StravaProvider = ({ children }) => {
  ...
  const loadStravaTokens = async () => {
    try {
      ...
      if (data) {
        ...
        console.log('- Config ID:', data.strava_config_id)
        ...
        try {
          ...
          const profile = await getAthleteProfile(data.access_token, user.id)
  ...
  const loadSyncPreferences = async () => {
    try {
      ...
      setSyncPreferences(data)
    } catch (error) {
      console.error('Error loading sync preferences:', error)
    }
  }

  const loadPendingSyncRequests = async () => {
    try {
  ...
  const handleTokenRefresh = async (refreshToken) => {
    try {
      ...
      const profile = await getAthleteProfile(tokens.access_token, user.id)
  ...
  const saveStravaTokens = async (tokens) => {
    try {
      const tokenData = {
        ...
        strava_config_id: tokens.strava_config_id || null,
  ...
  const fetchActivities = async (page = 1, perPage = 30, forceRefresh = false) => {
    ...
    // Check if manual sync approval is required
    const manualSyncRequired = await requiresManualSyncApproval(user.id)
    if (manualSyncRequired && !forceRefresh) {
      console.log('Manual sync approval required, not fetching activities automatically')
    ...
    try {
      const activitiesData = await getAthleteActivities(stravaTokens.access_token, page, perPage, user.id)
      ...
      if (error.message === 'Token expired') {
        try {
          ...
          const activitiesData = await getAthleteActivities(refreshedTokens.access_token, page, perPage, user.id)
          setActivities(activitiesData)
          setLastFetchTime(now)
          return activitiesData
        } catch (refreshError) {
  ...
  const requestManualSync = async (syncType = 'activities_sync') => {
    try {
      setLoading(true)
      const request = await requestSyncApproval(user.id, syncType)
      await loadPendingSyncRequests() // Refresh pending requests
      return request
    } catch (error) {
      setError(error.message)
      throw error
  ...
  const approveManualSync = async (requestId) => {
    try {
      await updateSyncRequestStatus(requestId, 'approved')
      await loadPendingSyncRequests() // Refresh pending requests

      // Execute the sync immediately
      const request = pendingSyncRequests.find(r => r.id === requestId)
      if (request) {
        if (request.request_type === 'activities_sync' || request.request_type === 'full_sync') {
          await fetchActivities(1, 30, true) // Force refresh
        }
        if (request.request_type === 'profile_sync' || request.request_type === 'full_sync') {
          const profile = await getAthleteProfile(stravaTokens.access_token, user.id)
          setAthlete(profile)
        }
        await updateSyncRequestStatus(requestId, 'completed')
      }
    } catch (error) {
      await updateSyncRequestStatus(requestId, 'failed', error.message)
      setError(error.message)
      throw error
    }
  }

  const updateSyncPreferences = async (preferences) => {
    try {
      const { data, error } = await supabase
        .from('user_sync_preferences')
        .upsert({
  ...
  const getStravaAuthUrlForUser = async () => {
    try {
      return await getStravaAuthUrl(user?.id)
    } catch (error) {
